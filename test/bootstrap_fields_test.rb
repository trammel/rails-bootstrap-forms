...
class BootstrapFieldsTest < ActionView::TestCase
  ...
  test "file fields can have span attributes set correctly" do
    ...
    assert_equivalent_xml expected, @builder.file_field(:misc, span: {attribute: 'value'})
  end
  ...
  test "password fields are wrapped correctly" do
    expected = %{<div class="form-group"><label class="form-control-label" for="user_password">Password</label><input class="form-control" id="user_password" name="user[password]" type="password" /><small class="form-text text-muted">A good password should be at least six characters long</p></div>}
